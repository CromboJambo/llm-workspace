//! tcgen05 GEMM for sm_100+ (Blackwell — B200 / RTX 5060 Ti / 5090)
//!
//! Phase 3: tensor core (tcgen05) with tensor memory (TMEM).
//! This is the speed-of-light GEMM approach from gemm_sol.
//!
//! Key differences from Phase 2 (WGMMA):
//! - Uses tensor memory (tm_mem) instead of shared memory as accumulator
//! - tcgen05 instructions read directly from TMEM (no register spill)
//! - 128x128x16 MMA tiles (vs 64x64x64 in WGMMA)
//! - TMA for async GMEM→SMEM copies
//! - SWIZZLE_128B for correct core matrix layout
//!
//! Build and run with:
//!   cargo oxide run tcgen05_gemm
...
pub unsafe fn tcgen05_gemm(
    ...
    unsafe {
        ...
        if c_row < m && c_col < n {
            // Allocate tensor memory (warp 0 only)
            if is_warp0 {
            ...
            let mut kk = 0usize;
            while kk < k {
                // Load A tile column into shared memory
                let a_col = kk + tid / BM;
                if a_col < k {
                ...
                // Load B tile row into shared memory
                let b_row = kk + tid % BM;
                if b_row < k {
                    let b_col = tile_n + tid / BN;
                    if b_col < n {
            ...
            *ptr.add(c_idx) = alpha * sum + beta * (*ptr.add(c_idx));
            ...
            // Deallocate tensor memory (warp 0 only)
            if is_warp0 {
...
fn main() {
    println!("=== tcgen05 GEMM (sm_100+ / Blackwell) ===");
    ...
    let module = ctx
        .load_module_from_file("tcgen05_gemm.ptx")
