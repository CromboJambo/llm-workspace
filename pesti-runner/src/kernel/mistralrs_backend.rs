...
mod tests {
    ...
    #[test]
    fn backend_default_is_mistralrs_if_cuda_available() {
        let backend = MistralRsBackend::default();
        if crate::cuda_runtime::is_available() {
            assert_eq!(backend, MistralRsBackend::MistralRs);
        } else {
            assert_eq!(backend, MistralRsBackend::Cpu);
        }
    }

    #[test]
    fn backend_description_formats_correctly() {
        assert_eq!(
            MistralRsBackend::MistralRs.description(),
            "mistral.rs (production GPU kernels)"
        );
        assert_eq!(
            MistralRsBackend::Cuda.description(),
            "PESTI CUDA (PTX, unverified)"
        );
        assert_eq!(
            MistralRsBackend::Cpu.description(),
            "CPU (reference)"
        );
    }

    #[test]
    fn backend_gpu_available_checks() {
        assert!(MistralRsBackend::MistralRs.gpu_available());
        assert!(MistralRsBackend::Cuda.gpu_available());
        assert!(!MistralRsBackend::Cpu.gpu_available());
    }

    #[test]
    fn gemm_kernel_creation_cpu() {
        let backend = MistralRsBackend::Cpu;
        let kernel = backend.create_gemm_kernel(GemmArch::default());
        assert!(kernel.is_some());
        assert!(kernel.unwrap().is_available());
    }
}
